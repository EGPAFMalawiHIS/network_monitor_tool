...
function gitInstallation {
    ...
    if ! [ -x "$(command -v git)" ]; then
        ...
        if ! [ -x "$(command -v git)" ]; then
            # install git from packages folder
            sudo dpkg -i ./packages/git-all_2.24.0-1~nd18.04+1_all.deb
            if ! [ -x "$(command -v git)" ]; then
                echo 'git installation failed.' >&2
                echo 'Please install git manually and try again.' >&2
                exit 1
            fi
        fi
    fi
...
function sqliteInstallation {
    ...
    if ! [ -x "$(command -v sqlite3)" ]; then
        ...
        if ! [ -x "$(command -v sqlite3)" ]; then
            # install sqlite3 from packages folder
            sudo dpkg -i packages/sqlite3_3.22.0-1_amd64.deb
            if ! [ -x "$(command -v sqlite3)" ]; then
...
function curlInstallation {
    ...
    if ! [ -x "$(command -v curl)" ]; then
        ...
        if ! [ -x "$(command -v curl)" ]; then
            # install curl from packages folder
            sudo dpkg -i packages/curl_7.58.0-2ubuntu3_amd64.deb
            if ! [ -x "$(command -v curl)" ]; then
...
function iperfInstallation {
    # check if iperf3 is installed
    if ! [ -x "$(command -v iperf3)" ]; then
        echo 'iperf3 is not installed.' >&2
        echo 'Installing iperf3...'
        sudo apt-get install iperf3
        # exit if iperf3 is not installed
        if ! [ -x "$(command -v iperf3)" ]; then
            # install iperf3 from packages folder
            sudo dpkg -i packages/iperf3_3.1.3-1_amd64.deb
            if ! [ -x "$(command -v iperf3)" ]; then
                echo 'iperf3 installation failed.' >&2
                echo 'Please install iperf3 manually and try again.' >&2
                exit 1
            fi
        fi
    fi
...
function capturEnv {
    ...
    read -p 'Enter Test Interval in seconds(minimum 60): ' duration
